...
class GradLoss(nn.Module):
    ...
    def get_grad(self, img):
        return torch.cat(((img[:,:,1:,:-1] - img[:,:,:-1,:-1]), \
                          (img[:,:,:-1,1:] - img[:,:,:-1,:-1])), dim = 1)
    def forward(self, x,y):
        return torch.nn.L1Loss()(self.get_grad(x), self.get_grad(y))
    

# --------------------------------------------
# Patterned flash reconstruction loss
# --------------------------------------------
class PFLoss_edge_dloss(nn.Module):
    # Loss used in MFFNet training

    def __init__(self, lam_vgg, lam_edge, lam_recp, lam_dloss, pattern_boost):
        super(PFLoss_edge_dloss, self).__init__()
