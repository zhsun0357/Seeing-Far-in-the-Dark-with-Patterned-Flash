...
class MFFNet_single(torch.nn.Module):
    def __init__(self, in_chans = 6, out_chans = 3, disp_clip = 1.0):
        super(MFFNet_single, self).__init__()
    ...
    def forward(self, X):
        ...
        sx = torch.nn.Tanh()(y[:,6,:,:])*self.disp_clip
        sy = torch.nn.Tanh()(y[:,7,:,:])*self.disp_clip
        pattern_shift = self.get_pattern_shift(X, sx, sy)
        y = torch.cat((y[:,:6,:,:], pattern_shift, \
