...
def parse(opt_path, is_train=True):
    ...
    # pdb.set_trace()
    if opt['restore']:
        path_task = os.path.join(opt['path']['root'], opt['path']['restore'])
    else:
        path_task = os.path.join(opt['path']['root'], opt['task'].replace('cnn', opt['netG']['net_type'])) \
                        + '_{:%m%d%H%M%S}'.format(datetime.now())
